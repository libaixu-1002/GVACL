...
N <- c(12,20,30,50,80,100,150,200,300)
...
for (ITER in ITER0:length(M)) {
  for (trial in trial0:TRIALS) {
    ...
    ###############################################################################
    save.image("Gam-S-M500-delta-1.Rdata")	
  }
}

load("C:\\Users\\DELL\\Desktop\\Resubmit-SS-R3\\Gamma\\Gam-S-M500-delta-1.Rdata")
###############################################################################
...
for (ITER in 1:(length(M))) {
  ...
  inds <- c(1:length(converged.gva[ITER,1:TRMT]))
  uncong.gva   <- which(converged.gva[ITER,1:TRMT] !=1)
  uncong.regva <- which(converged.regva[ITER,1:TRMT] !=1)  
  inds.outliers <- c()
...
data.gva0 <- data.frame(
  group = c(rep("216",   length(effsize[[1]])),
            rep("400",   length(effsize[[2]])),
            rep("900",   length(effsize[[3]])),
            rep("2500",  length(effsize[[4]])),
            rep("6400",  length(effsize[[5]])),
            rep("10000", length(effsize[[6]])),
            rep("22500", length(effsize[[7]])), 
            rep("40000", length(effsize[[8]])),
            rep("90000", length(effsize[[9]]))),
...
data.gva0$group <- factor(data.gva0$group,
                          levels = c("216","400","900","2500","6400","10000","22500","40000","90000")
)
...
beta0regvabox <- ggplot(data.regva0, aes(x=group, y=value, fill = group)) + 
  ...
  theme(legend.position = "top",
        axis.text.x = element_text(angle = 45,hjust = 1))
library(patchwork)
beta0gvabox  + beta0regvabox + plot_layout(ncol = 3)
...
beta0gvabox  + beta0regvabox +
  beta1gvabox + beta1regvabox + plot_layout(ncol = 2)
...
for(j in 1:p){
  for(i in 1:length(M)){
    ...
    #dim(theta.gva)
    UCI <- theta.gva[i,1:TRMT,j] + 1.96*asy.se.theta.regva[i,1:TRMT,j]
    LCI <- theta.gva[i,1:TRMT,j] - 1.96*asy.se.theta.regva[i,1:TRMT,j]
    UI <- which(UCI>=para[j])
    LI <- which(LCI<=para[j])
    CP[i,j] <- 1 - length(setdiff(UI,LI))/TRMT
  }
}
# 加载必要的库
library(ggplot2)
