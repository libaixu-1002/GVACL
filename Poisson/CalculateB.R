...
B12.fun <- function(family,vmuuv,muui,tui,muvj,tvj,gh) 
{
  ...
  if (famil =="GAMMA"){
    vB1 <- exp(-vmuuv - mui + 0.5*exp(tui) -muvj + 0.5*exp(tvj))
    vB2 <- vB1
  }
...
B1234.fun <- function(family,vmuuv,muui,tui,muvj,tvj,gh) 
{
  ...
  return(list(vB0=vB0,vB1=vB1,vB2=vB2,vB3=vB3,vB4=vB4))
}
...
{ 
  ...
  if (family=="LOGISTIC") {
    vB0 <- B0rc.aghq(mu,sigma2,gh)$vB0
  }
  if (family == "GAMMA") {
    vB0 <- exp(-mu + sigma2/2)
  }
...
B12rc.fun <- function(family,mu,sigma2,gh) 
{
  ...
  return(list(vB0=vB0,vB1=vB1,vB2=vB2))
}
