...
ReGVA.CLCR.f <- function(vtheta,family,vy,mX,gh){
  ...
  for (i in 1:m) {
    ...
    elbo <- elbo + sum(vym[i,]*vmu) - sum(vB0) - 0.5*log(2*pi) - 0.5*tu - 0.5*(muui^2 + exp(tui))*exp(-tu) + 0.5*log(2*pi) + 0.5*tui + 0.5
    #elbo_1 <- elbo_1 + sum(vym[i,]*vmu) - sum(vB0)
  ...
  for(j in 1:n){
    ...
    elbo <- elbo + sum(vym[,j]*vmu) - sum(vB0) - 0.5*log(2*pi) - 0.5*tv - 0.5*(muvj^2 + exp(tvj))*exp(-tv) + 0.5*log(2*pi) + 0.5*tvj + 0.5
    
    #elbo_4 <- elbo_4 + sum(vym[,j]*vmu) - sum(vB0)
    #elbo_5 <- elbo_5 - 0.5*log(2*pi) - 0.5*tv - 0.5*(muvj^2 + exp(tvj))*exp(-tv)
    #elbo_6 <- elbo_6 + 0.5*log(2*pi) + 0.5*tvj + 0.5
  }
  ...
  if (family=="POISSON"){
    for(i in 1:m){
      elbo <- elbo - sum(lgamma(vym[i,]+1))
    }
    for(j in 1:n){
      elbo <- elbo - sum(lgamma(vym[,j]+1))
    }
  }
...
{ 
  ...
  for(i in 1:m){
    ...
    ELBO_beta1 <- ELBO_beta1 + apply(midtemp,2,sum)
  ...
  for(j in 1:n){
    ...
    midtemp <- as.matrix((vym[,j]-res$vB1)*mX[,j,],m,p-1)
    ELBO_beta1 <- ELBO_beta1 + apply(midtemp,2,sum)
...
ReGVA.CLCR.vgANDmH <- function(vtheta,family,vy,mX,gh){ 
  ...
  for(i in 1:m){
    ...
    ELBO_betabeta[1,3:(p+1)] <- ELBO_betabeta[1,3:(p+1)] - apply(midtemp,2,sum) 
  ...
  for(j in 1:n){
    ...
    ELBO_betabeta[2,3:(p+1)] <- ELBO_betabeta[2,3:(p+1)] - apply(midtemp,2,sum)
  ...
  for(i in 1:m){
    ...
    midtemp <- as.matrix(mX[i,,]*res$vB2,n,p-1)
    ELBO_beta1muui <- -apply(midtemp, 2, sum)
    ELBO_betartui  <- -sum(res$vB3)*0.5*exp(tui)
    
    midtemp <- as.matrix(mX[i,,]*res$vB3,n,p-1)
    ELBO_beta1tui  <- -apply(midtemp,2,sum)*0.5*exp(tui)
  ...
  for(j in 1:n){
    ...
    midtemp <- as.matrix(mX[,j,]*res$vB2,m,p-1)
    ELBO_beta1muvj <- -apply(midtemp, 2, sum)
    ELBO_betactvj  <- -sum(res$vB3)*0.5*exp(tvj)
    ELBO_beta1tvj  <- -apply(midtemp,2,sum)*0.5*exp(tvj)
...
ReGVA.CLCR.FIT <- function(vbeta,vSigma,lumu,luLambda,lvmu,lvLambda,family,vy,mX,id) 
{
  ...
  return(list(NI=NI,vbeta=vbeta,sigma2=exp(sigmauv),muu=vmuu,lambdau=exp(vzetau),muv=vmuv,lambdav=exp(vzetav),vbeta.serr=sqrt(diag(res$sI)),sl=sl))
}
