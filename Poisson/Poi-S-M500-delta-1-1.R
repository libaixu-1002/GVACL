########## Simulation for Poisson case ##########
# Last changed: 20 January. 2025

###############################################################################
rm(list = ls())
##Poisson case##
setwd("C:\\Users\\DELL\\Desktop\\Resubmit-SS-R3\\Poisson")
library(Matrix)
library(MASS)
library(statmod)
library(mvQuad)
  ...
  for (trial in trial0:TRIALS) {
    ...
    if (do.GVACRCL){
      ...
      if ((class(res)[1])!="try-error") {	
        ...
        for(i in 1:m){
          for(j in 1:n){
            sumx[i,j] <-  sum(mX[i,j,]*res$vbeta[3:(p+1)])
          }
        }
        
        expbeta1x <- exp(sumx) #expbeta1x <- exp(res$vbeta[3]*mX)
        exp2beta1x <- exp(2*sumx)
        phi0beta1 <- mean(expbeta1x)
        phi02beta1 <- mean(exp2beta1x)
        
        xexpbeta1x  <- matrix(0,m*n*(p-1),1); dim(xexpbeta1x) <- c(m,n,p-1)
        xexp2beta1x <- matrix(0,m*n*(p-1),1); dim(xexp2beta1x) <- c(m,n,p-1)
        phi1beta1 <- c()
        phi12beta1 <- c()
        for(i in 1:(p-1)){
          xexpbeta1x[,,i] <- mX[,,i]*expbeta1x 
          phi1beta1 <- c(phi1beta1,mean(xexpbeta1x[,,i]))
          xexp2beta1x[,,i] <- mX[,,i]*exp2beta1x
          phi12beta1 <- c(phi12beta1,mean(xexp2beta1x[,,i]))
        }
        
        x2expbeta1x <- matrix(0,m*n*(p-1)*(p-1),1); dim(x2expbeta1x) <- c(m,n,p-1,p-1)
        x2exp2beta1x <- matrix(0,m*n*(p-1)*(p-1),1); dim(x2exp2beta1x) <- c(m,n,p-1,p-1)
        
        for(i in 1:m){
          for(j in 1:n){
            x2expbeta1x[i,j,,]  <- mX[i,j,]%*%t(mX[i,j,]*expbeta1x[i,j])
            x2exp2beta1x[i,j,,] <- mX[i,j,]%*%t(mX[i,j,]*exp2beta1x[i,j])
          }
        }
        
        phi2beta1 <- matrix(0,p-1,p-1)
        phi22beta1 <- matrix(0,p-1,p-1)
        for(i in 1:(p-1)){
          for(j in 1:(p-1)){
            phi2beta1[i,j] <- mean(x2expbeta1x[,,i,j])
            phi22beta1[i,j] <- mean(x2exp2beta1x[,,i,j])
          }
        }
        
        gammabeta1 <- phi1beta1/phi0beta1
        tau <- phi1beta1^2/(phi2beta1*phi0beta1-phi1beta1^2)
        a11 <- 2*(exp(res$sigma2[2])-1)
        a12 <- 2*res$sigma2[2]
        a13 <- -res$sigma2[2]^2
        a22 <- 2*res$sigma2[2]
        a33 <- res$sigma2[2]^2
        Gamma1 <- matrix(c(a11,a12,a13,a12,a22,0,a13,0,a33),3,3) 
        b11 <- 2*(exp(res$sigma2[1])-1)
        b21 <- 2*res$sigma2[1]
        b31 <- -res$sigma2[1]^2
        b22 <- 2*res$sigma2[1]
        b33 <- res$sigma2[1]^2
        Gamma2 <- matrix(c(b11,b21,b31,b21,b22,0,b31,0,b33),3,3) 
        I <- c(1,1,1)
        sebeta0 <- sqrt(t(I)%*%Gamma1%*%I/(8*n)+t(I)%*%Gamma2%*%I/(8*m))
        tau1 <- solve(phi2beta1*phi0beta1-phi1beta1%*%t(phi1beta1))
        tau2 <- phi2beta1-gammabeta1%*%t(phi1beta1)-phi1beta1%*%t(gammabeta1) + gammabeta1%*%t(gammabeta1)*phi0beta1
        tau3 <- phi22beta1 - gammabeta1%*%t(phi12beta1) - phi12beta1%*%t(gammabeta1)  + gammabeta1%*%t(gammabeta1)*phi02beta1
        coff1 <- phi0beta1^2*exp(-0.5*res$vbeta[1]-0.5*res$vbeta[2]-res$sigma2[1]/4-res$sigma2[2]/4)
        coff2 <- (4*exp(res$sigma2[1]+res$sigma2[2])-3*exp(res$sigma2[1])-3*exp(res$sigma2[2]) + 2)*0.25*phi0beta1^2
        
        var1 <- coff1*tau1%*%tau2%*%tau1
        var2 <- coff2*tau1%*%tau3%*%tau1
        
        SSigma <- (var1+var2)/(m*n)
        sebeta1  <- sqrt(diag(SSigma))
    ...
    if(do.Laplace){
      ...
      bval3 <- proc.time()
      dat$vx.1
      m3 <- try(Lap.fit <- glmmTMB(y ~ vx.1 + vx.2 + vx.3 + vx.4 + vx.5 + vx.6 + vx.7 + vx.8 + (1|id) + (1|sample), ziformula=~0, family=poisson, data=dat), silent=TRUE)
      eval3  <- proc.time()
    ...
    save.image("Poi-S-M500-delta-1-1.Rdata")	
  }
}
load("C:\\Users\\DELL\\Desktop\\Resubmit-SS-R3\\Poisson\\Poi-S-M500-delta-1-1.Rdata")
...
plot1 <- ggplot() + 
  ...
  scale_y_continuous(
    ...
    breaks = c(1e-2,1e-1, 1e0, 1e1, 1e2),  # 设置刻度
  ...
  labs(
    title = "CPU sec. Poisson",     # 图表标题
    x = "Sample size",              # 横轴标签
    y = "Time/s",                   # 纵轴标签
    color = "Method"                # 图例标题
  )+
  labs(title = "CPU sec. Poisson", x= "Sample size", y="Time/s") +
  theme_minimal() + 
...
data.lap0 <- data.frame(
  ...
  value = c(theta.lap[1,effsize[[1]],1], 
            ...
            theta.lap[5,effsize[[5]],1],
            theta.lap[6,effsize[[6]],1],
            theta.lap[7,effsize[[7]],1],
            theta.lap[8,effsize[[8]],1],
            theta.lap[9,effsize[[9]],1])
)
...
data.lap1 <- data.frame(
  ...
  value = c(theta.lap[1,effsize[[1]],2], 
            ...
            theta.lap[5,effsize[[5]],2],
            theta.lap[6,effsize[[6]],2],
            theta.lap[7,effsize[[7]],2],
            theta.lap[8,effsize[[8]],2],
            theta.lap[9,effsize[[9]],2])
)
...
data.lap2 <- data.frame(
  ...
  value = c(theta.lap[1,effsize[[1]],3], 
            ...
            theta.lap[5,effsize[[5]],3],
            theta.lap[6,effsize[[6]],3],
            theta.lap[7,effsize[[7]],3],
            theta.lap[8,effsize[[8]],3],
            theta.lap[9,effsize[[9]],3])
)
...
sigma1gvabox <- ggplot(data.gvau, aes(x=group, y=value, fill = group)) + 
  ...
  labs(title=expression(sigma[u]~"GVA"), x="Sample size ", y= expression(sigma[1])) +
  theme_minimal() +
  theme(legend.position = "top",
        axis.text.x = element_text(angle = 45,hjust = 1))
...
data.lapu <- data.frame(
  ...
  value = c(theta.lap[1,effsize[[1]],p+1],
            ...
            theta.lap[5,effsize[[5]],p+1],
            theta.lap[6,effsize[[6]],p+1],
            theta.lap[7,effsize[[7]],p+1],
            theta.lap[8,effsize[[8]],p+1],
            theta.lap[9,effsize[[9]],p+1])
)
...
data.lapv <- data.frame(
  ...
  value = c(theta.lap[1,effsize[[1]],p+2],
            ...
            theta.lap[5,effsize[[5]],p+2],
            theta.lap[6,effsize[[6]],p+2],
            theta.lap[7,effsize[[7]],p+2],
            theta.lap[8,effsize[[8]],p+2],
            theta.lap[9,effsize[[9]],p+2])
)
...
# 创建图形
PoiCPplot <- ggplot(data, aes(x = m, y = coverage, linetype = parameter)) +
  geom_line() +
  geom_hline(yintercept = 95, linetype = "solid", color = "grey", size = 1) +  # 加粗参考线
  labs(
    x = "sample size",
    y = "coverage percentage",
    title = "Poisson case"~~~~X[ij] %~% N(0, 1),
